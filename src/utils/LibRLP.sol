...
/// @author https://github.com/artgobblers/art-gobblers/blob/master/test/utils/LibRLP.sol
library LibRLP {
    ...
    function computeAddress(address deployer, uint256 nonce) public pure returns (address) {
        ...
        if (nonce == 0x00) {
            return keccak256(abi.encodePacked(bytes1(0xd6), bytes1(0x94), deployer, bytes1(0x80)))
                .fromLast20Bytes();
        }
        if (nonce <= 0x7f) {
            return keccak256(abi.encodePacked(bytes1(0xd6), bytes1(0x94), deployer, uint8(nonce)))
                .fromLast20Bytes();
        }
